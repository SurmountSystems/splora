...
- **Flake and NixOS module.** `flake.nix` and `nix/module.nix` export `nixosModules.splora`, crane packages `splora` and `splora-liquid`, overlays, five-instance eval checks, one-instance remote JSON-RPC eval (`nixosRemoteJsonrpcImport`: cookie path plus rpc addr plus `--jsonrpc-import`, no local datadir), queue listen XOR socket, optional popular-scripts timer, and named check `rocksdbMoldLink`. The flake sets `useSystemRocksdb = false` (bundled rocksdb 0.24.0). 2026-09-01 `just check-remote` failed in `splora-deps` because gcc rejected mold `-fuse-ld=` as a store path. That was not a Menhera DNS miss. Do not claim system RocksDB is linked. Fallback is in [doc/supply-chain.md](doc/supply-chain.md).
...
## 5. HTTP/2 and HTTP/3 (not this tree)

HTTP/2 and HTTP/3 do **not** terminate in this crate. They terminate on the Surmount HTTPS edge in another tree (surmount-server), on first-party Axum: TCP ALPN `h2` plus HTTP/1.1, and UDP QUIC ALPN `h3`. The named knobs there are `surmount.sploraProxy` (Host to Unix socket reverse proxy) and `surmount.managementUi.http3Enable` (UDP QUIC on the HTTPS port). On surmount-1, `surmount.sploraProxy` is already enabled. `http3Enable` stays true. This session did not edit that tree.

The NixOS option `services.splora.instances.<name>.httpSocketFile` already defaults to `/run/splora/${name}.http.sock`. Do not change that default to TCP. Splora HTTP for the edge binds that path only. Electrum newline stays on `/run/splora/${name}.electrum.sock`. The edge must not point at the Electrum socket.

Indexer HTTP, queue HTTP, and Electrum-over-HTTP on splora unix sockets are local cleartext **HTTP/1.1**. QUIC cannot sit on a Unix domain socket. Do not put QUIC on a unix socket. When a client arrives on HTTP/2 (TCP ALPN `h2`) or HTTP/3 (QUIC ALPN `h3`) at the edge, that edge still talks HTTP/1.1 to `/run/splora/<instance>.http.sock` and `/run/splora/queue.sock`.

Integration in the other tree has already started: `SURMOUNT_SPLORA_PROXY`, `SURMOUNT_SPLORA_INSTANCES`, `SURMOUNT_SPLORA_SOCKET_DIR`, `SURMOUNT_SPLORA_QUEUE_SOCKET`, and `SURMOUNT_SPLORA_QUEUE_PATH`. Instance names are `mainnet`, `testnet3`, `testnet4`, `mutinynet`, and `liquid`. That edge refuses Electrum newline Unix sockets on the HTTP proxy (`SURMOUNT_SPLORA_ELECTRUM_SOCKET` set is fail-closed; paths that look like Electrum sockets are refused). Socket Electrum stays on `/run/splora/<instance>.electrum.sock` and is not an HTTP/2 or HTTP/3 surface.

There is no nginx in this repository. Do not add HTTP/2 or HTTP/3 to hyper in this crate to match the public edge. README points at this section. Socket paths and the NIP-98 `X-Forwarded-Proto` contract are in [README.md](README.md) under **Surmount edge**.
