...
- REST body and time valves: request body collection times out after 30 seconds (`REQUEST_BODY_TIMEOUT`) and rejects bodies larger than 1,000,000 bytes (`MAX_BODY_SIZE`) with HTTP 413. A Blockstream-style HTTP/1 header-read timeout is **not** in this tree. The lockfile has hyper **0.14.32**, which has `Server::http1_header_read_timeout` (that API landed in 0.14.20). This tree did not wire that timeout. Do not document that header timeout as shipped.
...
HTTP/2 and HTTP/3 do **not** terminate in this crate. They terminate on the Surmount HTTPS edge in another tree (surmount-server), on Axum. The named knobs there are `surmount.sploraProxy` (Host to Unix socket reverse proxy, default off) and `surmount.managementUi.http3Enable` (UDP QUIC on the HTTPS port, default on when listen is https). This session did not edit that tree.

Indexer HTTP, queue HTTP, and Electrum-over-HTTP on splora unix sockets are local cleartext **HTTP/1.1**. QUIC cannot sit on a Unix domain socket. When a client arrives on HTTP/2 (TCP ALPN `h2`) or HTTP/3 (QUIC ALPN `h3`) at the edge, that edge still talks HTTP/1.1 to `/run/splora/<instance>.http.sock` and `/run/splora/queue.sock`.
